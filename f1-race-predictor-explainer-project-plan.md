...
### Phase 6, as actually built (2026-09-11) — done, one real architecture deviation from "tool calls" above

The plan's own phrase "live data and tool calls" implied a ReAct-style agent where the LLM decides which tool to invoke. Before building that, the local Llama 3.2 1B Instruct's LangChain tool-calling was tested for real (`ChatHuggingFace.bind_tools()`) — it came back with **zero tool calls**, and the underlying prompt showed no tool schema had even reached the model. Not a close call, not worth trying to coax into working: the graph does the multi-step orchestration itself instead, and only asks the LLM for the two things it's actually reliable at — full reasoning at the deterministic-tool-selection level of "LangGraph" (a real `StateGraph`, conditional routing, checkpointed memory), just not free-form LLM-driven tool choice.

- **`src/agent/f1_api.py`** — thin Jolpica-F1 (Ergast-compatible) client: driver standings, constructor standings, season schedule (including which remaining rounds are sprint weekends, needed for points math).
- **`src/agent/scenarios.py`** — the actual "what does X need to do to win" math: remaining-points pool, mathematical-elimination check, points needed to draw level assuming the rival scores nothing more. Deliberately does not implement full FIA countback tie-break rules (equal-points ties broken by most wins, etc.) — documented as a known simplification, not silently ignored.
- **`src/agent/graph.py`** — the `StateGraph`: `classify` (heuristic keyword match first, LLM only as a last resort — see LEARNING.md for why that's flipped from the original LLM-first design) routes to one of `fetch_driver_standings` / `fetch_constructor_standings` / `compute_title_scenario` / straight to `synthesize_answer`. The three structured intents get a **deterministic, templated answer directly from real computed data, no LLM in that path** — a real test showed the base model hallucinating even when handed correct data verbatim (a fabricated circuit feature, an invented driver code). Only the `general` catch-all still goes through the LLM, with the same honest quality caveat as `rag/chat.py`'s open-ended path.
- **Memory**: `langgraph.checkpoint.memory.MemorySaver` + a `thread_id`, with an explicit continuation-marker check ("and what about...") before a follow-up question inherits the previous turn's intent — an earlier version inherited for *any* unmatched question and wrongly answered a completely unrelated question with stale data from two turns earlier.
- **Output achieved:** yes, verified against real Jolpica-F1 data — driver standings, constructor standings, and the plan's own "what does Norris need to win the title" scenario all produce grounded, arithmetically-correct answers; follow-ups correctly reuse context.
